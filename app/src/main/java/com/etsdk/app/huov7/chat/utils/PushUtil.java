...
public class PushUtil implements Observer {
    ...
    private void PushNotify(TIMMessage msg){
        ...
        NotificationManager mNotificationManager = (NotificationManager) AileApplication.getInstance().getSystemService(AileApplication.getInstance().NOTIFICATION_SERVICE);
        NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(AileApplication.getInstance());
        Intent notificationIntent = new Intent(AileApplication.getInstance(), MainActivity2.class);
        notificationIntent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP
                | Intent.FLAG_ACTIVITY_SINGLE_TOP);
        PendingIntent intent = PendingIntent.getActivity(AileApplication.getInstance(), 0,
...
//                .setNumber(++pushNum) //设置通知集合的数量
    ...
    public void reset(){
        NotificationManager notificationManager = (NotificationManager)AileApplication.getInstance().getSystemService(AileApplication.getInstance().NOTIFICATION_SERVICE);
