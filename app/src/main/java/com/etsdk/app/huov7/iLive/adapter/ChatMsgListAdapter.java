package com.etsdk.app.huov7.iLive.adapter;
...
import com.etsdk.app.huov7.iLive.views.customviews.CustomTextView;
...
public class ChatMsgListAdapter extends BaseAdapter implements AbsListView.OnScrollListener {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (item.getType() == Constants.GIFT_TYPE) {
            ...
            holder.invite_ll = (LinearLayout) convertView.findViewById(R.id.invite_ll);
        }
        ...
        if (item.getType() == Constants.APPLY_VOICE) {
            holder.invite_ll.setVisibility(View.VISIBLE);
            holder.invite_ll.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (itemListener != null){
                        itemListener.applyVoice(item.getIdentifier());
                    }
                }
            });
        }
        ...
        ImgUtil.setImg(context, item.getFaceUrl(), R.mipmap.icon_load, holder.iv_game_img);
        String name = UIUtils.getLimitString(item.getSenderName(), 6);
    ...
    public void playDisappearAnimator(int pos, View view) {
        ...
        if (firstVisable <= pos) {
            ...
            L.d(TAG, "playDisappearAnimator->unexpect pos: " + pos + "/" + firstVisable);
        }
    }
        ...
        if (animatorIdx < MAXANIMATORCOUNT) {
            ...
            if (position < mAnimatorInfoList.size()) {
                ...
                if (during < 0) {   // 剩余时长小于0直接设置透明度为0并返回
                    ...
                    L.v(TAG, "continueAnimator->already end animator:" + position + "/" + item.getContext() + "-" + during);
                    return;
                }
            }

            // 创建属性动画并播放
            L.v(TAG, "continueAnimator->pos: " + position + "/" + listMessage.size() + ", alpha:" + startAlpha + ", dur:" + during);
            playViewAnimator(itemView, startAlpha, during);
        } else {
            L.v(TAG, "continueAnimator->ignore pos: " + position + "/" + listMessage.size());
        }
    }
    ...
    private void playDisappearAnimator() {
        for (int i = 0; i < mListView.getChildCount(); i++) {
            ...
            if (null == itemView) {
                L.w(TAG, "playDisappearAnimator->view not found: " + i + "/" + mListView.getCount());
                break;
            }
            ...
            if (position < mAnimatorInfoList.size()) {
                ...
                L.e(TAG, "playDisappearAnimator->error: " + position + "/" + mAnimatorInfoList.size());
            }
    ...
    private void clearFinishItem() {
        ...
        while (mAnimatorInfoList.size() >= listMessage.size()) {
            L.e(TAG, "clearFinishItem->error size: " + mAnimatorInfoList.size() + "/" + listMessage.size());
            if (mAnimatorInfoList.size() > 0) {
                mAnimatorInfoList.remove(0);
            } else {
                break;
    ...
    public void notifyDataSetChanged() {
        L.v(TAG, "notifyDataSetChanged->scroll: " + mScrolling);
        if (mScrolling) {
            // 滑动过程中不刷新
